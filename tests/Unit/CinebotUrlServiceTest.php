<?php
/**
 * Cinebot URL service unit tests.
 *
 * @package CinebotWp
 */
...
/**
 * Verifies deterministic and safe Cinebot URL construction.
 */
final class CinebotUrlServiceTest extends TestCase {
	...
	public function test_non_positive_flag_returns_null_without_other_validation(): void {
		$service = new CinebotUrlService();

		self::assertNull( $service->buildLocandina( 'https://invalid/?secret=1', '../invalid', 0, 0 ) );
		self::assertNull( $service->buildLocandina( '', '', -1, -5 ) );
	}
	...
	public function test_build_returns_exact_sample_url(): void {
		...
		self::assertSame(
			...
			$service->buildLocandina( 'ticket.cinebot.it', 'martinovich', 491, 1 )
		);
	}
	...
	public function test_build_normalizes_host_and_surrounding_path_slashes(): void {
		...
		self::assertSame(
			'https://ticket.cinebot.it/martinovich/titolo/491/locandina',
			$service->buildLocandina( 'TICKET.CINEBOT.IT', '///martinovich///', 491, 2 )
		);
	}
	...
	public function test_build_encodes_each_safe_nested_path_segment(): void {
		...
		self::assertSame(
			...
			$service->buildLocandina( 'ticket.cinebot.it', 'cinema uno/sala+due', 491, 1 )
		);
	}
	...
	public function test_build_double_encodes_percent_signs_to_prevent_encoded_traversal(): void {
		...
		self::assertSame(
			...
			$service->buildLocandina( 'ticket.cinebot.it', '%2e%2e/poster', 491, 1 )
		);
	}
	...
	public function test_build_is_deterministic(): void {
		$service = new CinebotUrlService();
		$first   = $service->buildLocandina( 'TICKET.CINEBOT.IT', '/cinema uno/poster/', 491, 1 );

		self::assertSame( $first, $service->buildLocandina( 'TICKET.CINEBOT.IT', '/cinema uno/poster/', 491, 1 ) );
	}
	...
	public function test_build_accepts_63_byte_dns_label(): void {
		...
		self::assertSame(
			...
			( new CinebotUrlService() )->buildLocandina( $host, 'poster', 491, 1 )
		);
	}
	...
	public function test_positive_flag_rejects_non_positive_title_id( int $title_id ): void {
		...
		( new CinebotUrlService() )->buildLocandina( 'ticket.cinebot.it', 'martinovich', $title_id, 1 );
	}
	...
	public function test_positive_flag_rejects_invalid_host( string $host ): void {
		try {
			( new CinebotUrlService() )->buildLocandina( $host, 'martinovich', 491, 1 );
	...
	public function test_positive_flag_rejects_invalid_path( string $path ): void {
		try {
			( new CinebotUrlService() )->buildLocandina( 'ticket.cinebot.it', $path, 491, 1 );
